		...
		private void txtSearch_KeyUp(object sender, KeyEventArgs e)
		{
			if (e.KeyCode == Keys.Enter) {
				...
				if (this.txtSearch.TextLength == 0) {
					this.view.RowFilter = String.Empty;
					return;
				}
				...
				foreach (DataColumn column in this.view.Table.Columns) 
					if(column.DataType == typeof(string)) {
						pattern = (filterExpression.Length > 0) ? "OR {0} LIKE '*{1}*'" : "{0} LIKE '*{1}*'"; 
						filterExpression.AppendFormat(pattern, column.ColumnName, txtSearch.Text);
					}
				
				this.view.RowFilter = filterExpression.ToString();
				this.grid.Focus();
			}
		}
